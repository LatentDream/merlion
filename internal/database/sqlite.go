...
// It also ensures that the directory for the database file exists, creating it if necessary.
func GetDBPath() (string, error) {
	if dbPath := os.Getenv("MERLION_DB_PATH"); dbPath != "" {
...
// It returns the initialized *sql.DB object or an error if any step fails.
func InitDB() (*sql.DB, error) {
	...
	if err := operations.ApplyMigrations(db); err != nil {
		db.Close() // Close the DB if migrations fail
